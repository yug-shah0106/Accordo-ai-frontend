...
//             <button className="px-2 pt-1 pb-0 text-sm bg-gray-200 rounded">D</button>
//             <button className="px-2 pt-1 pb-0 text-sm bg-gray-200 rounded">M</button>
//             <button className="px-2 pt-1 pb-0 text-sm bg-gray-200 rounded">Y</button>
//           </div>
...
const Bargraph = () => {
  ...
  const generateDummyData = (): Requisition[] => {
  ...
  const [filterType, setFilterType] = useState<FilterType>("Y"); // Default to Yearly view
  ...
  const filterData = (requisitions: Requisition[], type: FilterType): void => {
    const groupedData: Record<number | string, number> = {};
    let labels: string[] = [];
    ...
    if (type === "D") {
      ...
      labels = Object.keys(groupedData).sort((a, b) => Number(a) - Number(b));
      setBarData({
        labels,
        datasets: [
          {
