...
const LineGraphSaving = () => {
  ...
  const [filterType, setFilterType] = useState("D"); // Default to Day (D)
  ...
  const filterData = (requisitions, type) => {
    const savingsByCategoryAndTime = {};
    let labels = [];
    ...
    const categoryData = Object.keys(savingsByCategoryAndTime).map(
      (category, index) => ({
        label: category,
  ...
  const chartOptions = {
    ...
    plugins: {
      tooltip: {
        callbacks: {
          label: function (tooltipItem) {
            return `$${tooltipItem.raw.toFixed(2)}`; // Format savings as $
          },
        },
      },
    },
