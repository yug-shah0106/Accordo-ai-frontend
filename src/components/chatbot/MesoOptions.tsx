...
const MesoOptionCard: React.FC<{
  ...
  const emphasisKey =
    emphasisValue === "payment_terms" ? "payment" : emphasisValue;
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          {emphasisIcon && (
            <span className="text-sm opacity-70">{emphasisIcon}</span>
          )}
        </div>
        {isFinal && (
      ...
      <div className="grid grid-cols-2 gap-2 text-sm mb-3">
        <div className="flex justify-between">
          ...
          <span className="font-medium">
            {formatPrice(option.offer.total_price, currency)}
          </span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-500">Payment:</span>
          <span className="font-medium">
            {formatPaymentTerms(option.offer.payment_terms_days)}
          </span>
        </div>
...
export const MesoOptions: React.FC<MesoOptionsProps> = ({
    ...
    return (
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-yellow-700">
        ...
        <div className="text-sm mt-1">
          {mesoResult.reason || "Please continue with standard negotiation."}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">
            {showAsFinal
              ? "Final Offers - Select to Conclude"
      ...
      {/* Others Button - Show when not final MESO and callback is provided */}
      {!showAsFinal && onOthersClick && (
      ...
      <div className="text-xs text-gray-400 text-center mt-3">
        Your selection helps us understand your priorities for future
        negotiations
      </div>
    </div>
  );
