...
import { PiPlusSquareBold, PiDotsThreeBold } from "react-icons/pi";
  ...
  const actions = [
    {
      ...
      label: "Edit Details",
      icon: <VscEdit />,
      link: (row) => `/user-management/edit-user/${row.id}`,
    },
    {
      type: "button",
      label: "Change Status",
    ...
    try {
      ...
      setSelectedFilters((prevFilters) => [
        ...
        },
        {
          ...
          options: data?.data?.data?.map((role) => role.name),
        },
      ]);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      <div className="relative">
                        <PiDotsThreeBold 
                          ...
                          onClick={(event) => setSelectedRow({ element: event.currentTarget, user: row })}
                        />
                        <Menu
      ...
      {isModal && (
        ...
        >
          <p className="text-sm text-gray-600">Are you sure you want to reset the password for this user?</p>
        </Modal>
      )}
    </div>
  );
