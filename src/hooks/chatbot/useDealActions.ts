...
// Dev override: local Llama 3.1 8B (Ollama) typically takes 6-17s per response,
// which would trip the prod 5s budget on every turn. Production stays at 5s.
const PM_RESPONSE_TIMEOUT_MS = import.meta.env.DEV ? 20000 : 5000;
...
export function useDealActions(
  ...
  const vendorMode =
    (deal as unknown as { vendorMode?: boolean })?.vendorMode === true;
   ...
   */
  const sendVendorMessageTwoPhase = useCallback(
    async (content: string): Promise<void> => {
      if (!isValidContext(context)) {
        toast.error("Unable to send message: deal context is incomplete");
      ...
      try {
        ...
        const phase1Response = await chatbotService.saveVendorMessageInstant(
          context,
          content,
        );
        ...
        logger.debug(
          "[TwoPhase] Phase 2: Generating PM response (5s timeout), vendorMessageId:",
          vendorMessageId,
        );
        setPmTyping(true); // Show typing indicator

        // Create a promise that races between async response and timeout
        const pmResponsePromise = chatbotService.generatePMResponseAsync(
          context,
          vendorMessageId,
        );
        ...
        if (result === "timeout") {
          ...
          logger.debug(
            "[TwoPhase] LLM timeout after 5s, using fallback response",
          );
          pmResponse = await chatbotService.generatePMResponseFallback(
            context,
            vendorMessageId,
          );
          usedFallback = true;
        ...
        if (pmMessage) {
          setMessages((prevMessages) => {
            ...
            if (exists) {
              logger.debug(
                "[TwoPhase] PM message already exists, skipping duplicate",
              );
              return prevMessages;
            }
            logger.debug(
              "[TwoPhase] Adding PM message to UI",
        ...
        if (decision?.action === "REDIRECT") {
          toast("Message was off-topic — redirected back to negotiation", {
            icon: "↩️",
          });
        } else if (decision?.action === "ERROR_RECOVERY") {
          toast("Something went wrong, but we recovered gracefully", {
            icon: "🛡️",
          });
        }
        ...
        if (isMountedRef.current) {
          ...
          setTimeout(() => {
            if (isMountedRef.current) {
              ...
              setPmTyping(false);
              logger.debug(
                "[TwoPhase] Cleared sending and pmTyping states, phase2Completed:",
                phase2Completed,
              );
            }
          }, 50);
  ...
  const sendVendorOffer = useCallback(
    async (content: string): Promise<void> => {
      if (!isValidContext(context)) {
        toast.error("Unable to send offer: deal context is incomplete");
        logger.error(
          "sendVendorOffer: Invalid context - missing or invalid rfqId/vendorId/dealId",
          context,
        );
        return;
      }
      ...
      try {
        ...
        const response = await chatbotService.sendVendorMessage(
          context,
          content,
        );
        ...
        if (response.data.pmDecision) {
          ...
          if (action === "COUNTER") {
            toast(`Buyer countered (${utilityPercent}% utility)`, {
              icon: "💬",
            });
          } else if (action === "ACCEPT") {
            toast.success(
              `Buyer accepted your offer! (${utilityPercent}% utility)`,
            );
  ...
  const initializeNegotiation = useCallback(async (): Promise<void> => {
    if (!isValidContext(context)) {
      ...
      logger.error(
        "initializeNegotiation: Invalid context - missing or invalid rfqId/vendorId/dealId",
        context,
      );
      return;
    }
    ...
    try {
      ...
      // Notify user
      toast.success(
        "Negotiation started! The buyer has made their opening offer.",
      );
    } catch (err) {
      const errorMessage =
  ...
  const reset = useCallback(async (): Promise<void> => {
    if (!isValidContext(context)) {
      ...
      logger.error(
        "reset: Invalid context - missing or invalid rfqId/vendorId/dealId",
        context,
      );
      return;
    }
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to reset deal";
      toast.error(errorMessage);
